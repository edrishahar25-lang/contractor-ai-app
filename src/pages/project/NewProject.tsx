...
const propertySchema = z.object({
  ...
  // Optional numeric fields use .catch() to gracefully handle NaN/null from
  // autosaved drafts (JSON.stringify converts NaN → null on save).
  bathrooms: z.number().min(0).catch(1).default(1),
  toilets: z.number().min(0).catch(1).default(1),
  kitchens: z.number().min(0).catch(1).default(1),
  balconies: z.number().min(0).catch(0).default(0),
  ceilingHeight: z.number().positive().catch(2.7).default(2.7),
  floor: z.number().optional().catch(undefined),
